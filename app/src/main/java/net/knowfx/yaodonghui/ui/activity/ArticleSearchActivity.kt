...
class ArticleSearchActivity : BaseActivity() {
    ...
    override fun setData(savedInstanceState: Bundle?) {
        ...
        mBinding.value.refresh.bindController(true) {
            requestList(it)
        }
        mBinding.value.listRv.layoutManager = LinearLayoutManager(this)
        mBinding.value.listRv.adapter = mAdapter
    ...
    private fun requestList(page: Int) {
        ...
        when (mFlag) {
            "ZXH" -> {//真相汇
                mModel.value.searchTruthList(page, mKey)
            }

            "HQST" -> {//汇圈神探
                mModel.value.searchSherlockList(page, mKey)
            }

            "DSPH" -> {//毒蛇评汇
                mModel.value.searchSnakeList(page, mKey)
            }
